...
def writeForOneFile(fileDir: str, count: int, name, label):
    ...
    for fileName in os.listdir("./videoClassification/dataset/" + name + "/" + fileDir + "/frameResult/00020"):
        if "ppm" in fileName:
            img = cv2.imread('./videoClassification/dataset/' + name + '/' + fileDir + '/frameResult/00020/' + fileName, 0)
            videos.append(img)
