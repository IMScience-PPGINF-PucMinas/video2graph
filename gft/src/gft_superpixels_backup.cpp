...
namespace gft{
  namespace Superpixels{
	...
	for (i = 1; i < A->n; i++) {
	  ...
	  if(qx >= 0 && qx < label->ncols && qy >= 0 && qy < label->nrows){
	    q = qx + qy * label->ncols;
	    if (p == pred->data[q]) {
	      path.push(q);
	    } else {
	...
	for(i = 1; i < A->n; i++){
	  qx = px + A->dx[i];
	  qy = py + A->dy[i];
	  //if(Image32::IsValidPixel(label, qx, qy)){
	  if(qx >= 0 && qx < label->ncols && qy >= 0 && qy < label->nrows){
	    q = qx + qy*label->ncols;
	    if(Q->color[q] != BLACK){

	      wl = ColorSquaredDistance(cimg_lab, seeds[s], q);
	      //w = powf(wl, beta/2.0)*alpha_beta + dpq[i];
	      wl *= (wl*wl);
	      wl *= wl;
		...
		if (tmp > cost[q]) {
		  removeSubTree(q, label, Q, pred, cost, A);
		}
		else {
		  if (tmp == cost[q] && pred->data[q] == p &&
    ...
    Image32::Image32 *IFT_SLIC(CImage::CImage *cimg, bool colored,
      ...
      Image32::Image32 *label, *img = NULL, *grad = NULL;
      Heap::Heap *Q;
      Image32::Image32 *pred;
      float *cost;
      int N,ncols,nrows,S,p,q,i;
      SPixelSeed *seeds;
      int it, s, nseeds = 0;
      int p_x,p_y,q_x,q_y;
      AdjRel::AdjRel *A;
	...
	cimg_lab = CImage::Clone(cimg);
      ...
      grad = NULL; //Image32::SobelFilter(img);
      if(img != NULL) Image32::Destroy(&img);
      ...
      if(grad != NULL) Image32::Destroy(&grad);
